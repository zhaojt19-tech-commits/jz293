    ...
    for name_id in temp_list:
        ...
        for distance in distance_list:
            ...
            for id in range(num_id):
                ...
                # 加载训练好的模型参数
                checkpoint = torch.load(path_model + model_name, map_location={'cuda:2':device})
                ...
                with torch.no_grad():
                    for t, (images, labels) in enumerate(tqdm(test_loader)):
                        ...
                        # 得到恢复后的压缩图片
                        xhat = model.predict(feature_seed, labels, input_size, num_classes, return_latent_basal=False)
                        
                        # max = max.ravel().to(device)
                        # images = images.to(device)
                        # images = (images.reshape(input_size, -1) * max.reshape(-1, 1)).reshape(input_size, 1, shape[2], shape[3])
                        # xhat = (xhat.reshape(input_size, -1) * max.reshape(-1, 1)).reshape(input_size, 1, shape[2], shape[3])
                        ...
                        # 计算图片的分别MSE
                        # loss_MSE_sample = model.MSE_sample(images.reshape(input_size, 1, shape[2], shape[3]), xhat.reshape(input_size, 1, shape[2], shape[3]))
                        loss_MSE_sample = model.MSE_sample(images.reshape(input_size, 1, shape[2], shape[3]), xhat.reshape(input_size, 1, shape[2], shape[3]))
                        ...
                        # 计算图像平均SSIM
                        # ssim = ssim_calculate(images.reshape(input_size, 1, shape[2], shape[3]), xhat.reshape(input_size, 1, shape[2], shape[3]), window_size = 7)
                        ssim = ssim_calculate(images.reshape(input_size, 1, shape[2], shape[3]), xhat.reshape(input_size, 1, shape[2], shape[3]), window_size = 7)
                        SSIM_sum = SSIM_sum + ssim.item() * input_size
                        
                        # img_ori_sample = img_ori[sample_list]
                        # img_pre_sample = img_pre[sample_list]
                        ...
                        # loss_MSE_sample = loss_MSE_sample[sample_list]
                            ...
                            #     img_ori_sample = img_ori_sample.numpy()
                            #     img_pre_sample = img_pre_sample.numpy()
...
# f_data.close()
# f_average.close()
with open('data_CPA.json', 'w') as fp:
    json.dump(fig, fp)
