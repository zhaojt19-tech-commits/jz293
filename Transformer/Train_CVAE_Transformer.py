    ...
    for id in id_list:
        ...
        for e in range(epoch_num):
            ...
            for t, (images, labels, ts) in enumerate(tqdm(train_loader)):
                ...
                images = images.reshape(T, 1, shape[2], shape[3]).float().to(device)
                labels = labels.reshape(1, T, 1).to(device)
                ts = ts.reshape(1, T).to(device)
                optimizer.zero_grad()
                # forward函数输出
                xhat, mu, logvar, adv_cvae, adv_tr = model(xs=images, ts=ts, ys=labels, batch_size=T, mask=mask, is_label=False)
                # 计算损失
                ...
                KLD_loss = loss_dic['KLD']
                KLD_sum = KLD_sum + KLD_loss.item()*T
            ...
            print('After {} epoch, the average loss is {}, the average image MSE loss is {}, the average KLD loss is {}'.format(e+1+epochs_old, loss_average, MSE_average, KLD_average))
            print(torch.max(xhat).item())
