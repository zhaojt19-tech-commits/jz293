...
class CatConTransformer(nn.Module):
    ...
    def forward(self, xs: Tensor,  # T x 1 * H * W
        ...
        # print([torch.min(xhat).item(), torch.max(xhat).item()])
        # print([torch.min(xs).item(), torch.max(xs).item()])
        
        # 数据归一化
        # xhat = xhat.reshape(batch_size, -1)
        ...
        # xhat = xhat.reshape(batch_size, 1, self.image_H, self.image_W)
        
        # print([torch.max(xhat).item(), torch.max(xs).item()])
        # # 归一化处理
        # xhat = self.softmax(xhat.reshape(batch_size, -1)).reshape(batch_size, 1, self.image_H, self.image_W)

        return xhat, mu, logvar, adv_cvae, adv_tr

    def loss_fn(self, x, ys, xhat, mu, logvar, adv_cvae, adv_tr, beta=1.0, adv_cvae_weight=0, adv_tr_weight=0):
    ...
    def generate(self, xs, old_ts, new_ts, old_ys, new_ys, batch_size, old_mask, old_other_ts=None, new_other_ts=None,
        ...
        # print(torch.mean(z_dec))
    
        xgen = self.cvae_enc.decode(z_dec, new_ys, batch_size).view(T_new, 1, self.image_H, self.image_W)

        # print([torch.min(xgen).item(), torch.max(xgen).item()])
        # print([torch.min(xs).item(), torch.max(xs).item()])

        # # 数据归一化
        # xgen = xgen.reshape(batch_size, -1)
        ...
        # xgen = xgen.reshape(batch_size, 1, self.image_H, self.image_W)

        # print(torch.max(xgen).item())
        # print(torch.min(xgen).item())

        # # 归一化处理
        # xgen = self.softmax(xgen.reshape(batch_size, -1)).reshape(batch_size, 1, self.image_H, self.image_W)
