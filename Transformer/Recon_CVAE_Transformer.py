...
for name_id in temp_list:
    ...
    for id in id_list:
        ...
        SSIM_sum = 0
        with torch.no_grad():
            for t, (images, labels, ts) in enumerate(tqdm(train_loader)):
                # y = labels[0][0].item()
                T = labels.ravel().shape[0]
                input_size = T
                images = images.reshape(T, 1, shape[2], shape[3]).to(device)
                ...
                # forward函数输出
                xhat, mu, logvar, adv_cvae, adv_tr = model(xs=images, ts=ts, ys=labels, batch_size=T, mask=mask)
