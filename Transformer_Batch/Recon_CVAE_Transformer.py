...
shape_list = [(2099, 40, 75, 70), (2097, 69, 109, 89), (2088, 65, 132, 94), (2045, 40, 43, 67), (2073, 50, 43, 77),
              (2071, 50, 40, 68), (2079, 58, 41, 67)]
...
for name_id in temp_list:
    ...
    for id in id_list:
        ...
        dataset_train = Brain_Dataset(mode='train', name_id=name_id, seed=seed, proportion=proportion, squeeze=squeeze, num_slice=num_slice)
        ...
        # 定义模型
        model = CatConTransformer(input_size=[shape[2], shape[3]], input_channels=1, output_channels=1, num_classes=1,
                                    class_sizes=[shape[0]],
                                    latent_dim=latent_dim, temporal_dim=temporal_dim, cat_dims=cat_dims, num_heads=num_heads,
                                    num_ref_points=num_ref_points,
                                    tr_dim=tr_dim, adversarial_hidden_dims_encoder=adversarial_hidden_dims_encoder,
                                    latent_hidden_dims_encoder=latent_hidden_dims_encoder,
                                    cvae_yemb_dims=cvae_yemb_dims, learn_temporal_emb=learn_temporal_emb, minT=minT,
                                    maxT=maxT, device=device, num_cont=num_cont,
                                    other_temporal_dims=other_temporal_dims, other_minT=other_minT, other_maxT=other_maxT,
                                    cont_model=cont_model, rec_loss=rec_loss,
        ...
        with torch.no_grad():
            for t, (images, labels, ts, mask, num_non_blank) in enumerate(tqdm(train_loader)):
                ...
                loss_MSE_sample = model.MSE_sample(images, xhat, num_non_blank)
                ...
                # 计算图像平均SSIM
                images = images.reshape(N, num_slice, -1)
