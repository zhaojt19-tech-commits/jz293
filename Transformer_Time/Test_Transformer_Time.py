...
for name_id in range(2):
    ...
    for id in id_list:
        ...
        # 定义DataLoader
        dataset_test = Brain_Dataset(mode='test', transform=None, part=name, seed=seed, proportion=proportion, squeeze=squeeze)
        ...
        time_train = dataset_test.time_train
        ...
        SSIM_sum = 0
        with torch.no_grad():
            for t, (images, labels, ts, time) in enumerate(tqdm(test_loader)):
                T = labels.ravel().shape[0]
                input_size = T
                # 取出一个标签
                y = labels.ravel()[0]
                images_new = images.reshape(T, 1, shape[0], shape[1]).to(device)
                
                # 用于采样
                t_slice = ts.ravel()
                y_slice = labels.ravel()

                labels_new = labels.reshape(1, T, 1).int().to(device)
                ts_new = ts.reshape(1, T).to(device)
                time_new = time.reshape(1, T).to(device)
                max = max.to(device)
                # 将基因id转化为下标id
                y = gene_list[y.item()]

                images_old = torch.tensor(x_train[y].reshape(-1, 1, shape[0], shape[1])).to(device)
                labels_old = torch.tensor(labels_train[y].reshape(1, -1, 1)).to(device)
                ts_old = torch.tensor(ts_train[y].reshape(1, -1)).to(device)
                time_old = torch.tensor(time_train[y].reshape(1, -1)).to(device)
                ...
                batch_size=T, old_mask=None, old_other_ts=[time_old], new_other_ts=[time_new],
                compute_adv=False, k=1, get_zdec=False, old_uncertainties=None, new_uncertainties=None)


                # 计算损失
                loss_dic = model.loss_fn(x=images_new, ys=labels_new, xhat=xhat, mu=mu, logvar=logvar, adv_cvae=adv_cvae, adv_tr=adv_tr, beta=beta, adv_cvae_weight=adv_cvae_weight, adv_tr_weight=adv_tr_weight)
