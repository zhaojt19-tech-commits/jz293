...
path_average = './average_pred_CP.csv'
...
for name_id in temp_list:
    ...
    for id in id_list:
        ...
        test_loader = torch.utils.data.DataLoader(dataset_test, batch_size=cfg.batch_size, shuffle=True)
        ...
        with torch.no_grad():
            for t, (images, labels) in enumerate(tqdm(test_loader)):
                ...
                # # forward函数输出
                # output_list = model(feature_seed, [one_hot_0, one_hot_1])

                xhat = model.sample(num_samples=input_size, current_device=device, cats=[one_hot_0, labels[:, 1].reshape(-1, 1).float()])
                images = images.to(device)
                
                # max = max.reshape(-1, 1).to(device)
