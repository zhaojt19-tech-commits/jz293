...
class Encoder(nn.Module):
    ...
    def __init__(self, image_H, image_W, input_dim, conv_dims, fc_dim, latent_dim, num_classes):
        super().__init__()
    ...
    def forward(self, x, label0, label1, image_H, image_W, batch_size): # label0: batchz_size * 2001, label1: batch_size * 69
        # 标签拼接
...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim, image_H, image_W, conv_dims, output_dim, num_classes):
        super().__init__()

        # 考虑标签的加入
        latent_dim = latent_dim + num_classes[0] + num_classes[1]
        ...
        self.pred_layer = nn.Sequential(
            nn.ConvTranspose2d(prev_dim, output_dim, kernel_size=3, stride=2, padding=[2, 4], output_padding=0)
            # nn.Sigmoid()
        )

    def forward(self, x, label0, label1): # label0: batchz_size * 2001, label1: batch_size * 69
        # 加入标签
        x = torch.cat([x, label0], dim = 1)
        x = torch.cat([x, label1], dim = 1)
        x = self.fc(x)
...
class CVAE_onehot(nn.Module):
    """VAE"""

    def __init__(self, image_H, image_W, input_dim, conv_dims, fc_dim, latent_dim, num_classes):
        super().__init__()

        self.encoder = Encoder(image_H, image_W, input_dim, conv_dims, fc_dim, latent_dim, num_classes)
        self.decoder = Decoder(latent_dim, image_H, image_W, conv_dims, input_dim, num_classes)
    ...
    def forward(self, x, label0, label1, image_H, image_W, batch_size):
        mu, log_var = self.encoder(x, label0, label1, image_H, image_W, batch_size)
        z = self.sample_z(mu, log_var)
        recon = self.decoder(z, label0, label1)
        return recon, mu, log_var

    def generate(self, z, label0, label1):
        recon = self.decoder(z, label0, label1)
        return recon
